...
| 5.5 | **Top feedback + processing / success** | Full menu-bar / notch | On a notched display, show the red→orange wings. Otherwise use the corner pill. On release: collapse → blue processing shimmer → green "✓ inserted" state (events posted, not app-verified delivery). Note: idle = no bubble; never call it Apple Dynamic Island hardware. Status icon is the chakra, not a mic | Success end |
...
| 5.12 | **Privacy / local-first** | Settings General + Prompt toggles | Raw mode means no formatting LLM and no context sent to a formatter. **Fully local** requires Apple Silicon on-device MLX STT **and** formatting off **and** learning reviewer off; cloud STT still uploads audio. With cloud formatting on, transcript and allowed context leave for the formatter API; audio also leaves if cloud STT or audio-assisted formatting is enabled. History, dictionary, corrections, and—by default—per-dictation WAVs under `~/.golos/recordings/` are local files you can inspect or delete; keep-recordings is config-only | After toggle story |
| 5.13 | **Onboarding / settings tour** | Menu → Welcome / Setup…; then Settings tabs | 7-page wizard: permissions live ✓/✗, hold-key test pad, OpenRouter/local choice, formatting radio cards, try-it field, done. Settings: History, General (Fetch models lives here), Prompt, Learning, Dictionary — don't narrate every control | Exit Settings |
